...
Oracle JDBC 드라이버(`ojdbc8`)는 실행 JAR에 포함되어 있어 따로 준비하지 않아도 됩니다.

직접 빌드할 때만 추가로 필요합니다:

- Maven 3.x

SQL 검증은 보조 안전장치일 뿐이므로 Oracle 계정은 가능한 한 조회 전용 계정을 사용하세요.
...
성공하면 `ojdbc8` 드라이버가 포함된 실행 JAR(fat jar)이 생성됩니다.
...
실행 JAR과 프로젝트의 `modelconvertor.cmd`를 같은 폴더에 둡니다. JAR에 `ojdbc8` 드라이버가 포함되어 있어 별도 드라이버 파일이 필요 없습니다.
...
`modelconvertor.cmd`는 같은 폴더의 JAR를 `java -jar`로 실행합니다. `C:\tools\modelconvertor`를 `PATH`에 추가하거나 CMD 파일의 전체 경로를 사용하세요.

```powershell
C:\tools\modelconvertor\modelconvertor.cmd --help
```

실행 JAR은 소스 저장소에 커밋하지 않고 GitHub Release 에셋으로 배포합니다. 저장소를 clone한 뒤 포함된 `install.ps1`을 실행하면 Release에서 JAR을 내려받아 위 구성을 만들고 사용자 `PATH` 등록과 전역 스킬 설치까지 처리합니다.

```powershell
.\install.ps1
```

여러 Java 프로젝트에서 Claude Code 스킬로 사용하는 자세한 절차는 [전역 설치 가이드](docs/global-installation-guide.md)를 참고하세요.

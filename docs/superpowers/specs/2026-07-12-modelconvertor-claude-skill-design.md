...
7. CLI가 0이 아닌 종료 코드로 끝나면 종료 코드와 stderr의 오류 메시지를 누락하거나 임의로 바꾸지 않고 사용자에게 전달하며 자동으로 재시도하지 않는다. Oracle/JDBC 오류에 포함된 설정 비밀번호는 CLI가 `***`로 마스킹한다.
...
- 실패를 일반적인 오류 문구로 축약하거나 사용자 확인 없이 재시도하지 않는다.
...
- 0이 아닌 종료 코드와 stderr를 보존하고 자동 재시도하지 않는지 확인한다.
